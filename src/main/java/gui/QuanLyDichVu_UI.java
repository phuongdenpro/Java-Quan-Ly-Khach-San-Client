...
public class QuanLyDichVu_UI extends JFrame {
	...
	private JScrollPane scpTableDV;
	...
	public QuanLyDichVu_UI() throws MalformedURLException, RemoteException, NotBoundException {
		...
		scpTableDV = new JScrollPane(table);
		...
		btnTim.addActionListener((e) -> {
			if (txtTim.getText().equals("")) {
				...
				try {
					...
					String key = "maDV";
					if (cmbLoaiTimKiem.getSelectedItem().toString().equals("Mã dịch vụ")) {
						key = "maDV";
					} else if (cmbLoaiTimKiem.getSelectedItem().toString().equals("Tên dịch vụ")) {
						key = "tenDV";
					}

					dsdvtim = client.getDichVuDao().timKiemDichVu(key, txtTim.getText());
					if (dsdvtim.size() == 0) {
		...
		btnSua.addActionListener((e) -> {
			...
			DichVu dv = new DichVu(dsdv.get(idx).getMaDV(), txtTenDV.getText(),
...
//		table.getColumnModel().getColumn(2).setPreferredWidth(120);
	...
	public void clear() {
		...
		txtDonGia.setText("");
		txtTim.setText("");
		cmbLoaiTimKiem.setSelectedItem("Mã dịch vụ");
	}
	...
	public void renderDataDV() throws MalformedURLException, RemoteException, NotBoundException {
		...
		for (int j = 0; j < dsdv.size(); j++) {
			...
			modelTable
					.addRow(new Object[] { dichVu.getMaDV(), dichVu.getTenDV(), Currency.format(dichVu.getDonGia()) });
		}
		;
		table.revalidate();
		table.repaint();
	...
	public void renderDataTimKiem() throws SQLException {
		table.clearSelection();
		modelTable.getDataVector().removeAllElements();
		for (int j = 0; j < dsdvtim.size(); j++) {
			DichVu dichVu = dsdvtim.get(j);
			modelTable
					.addRow(new Object[] { dichVu.getMaDV(), dichVu.getTenDV(), Currency.format(dichVu.getDonGia()) });
		}
		;
